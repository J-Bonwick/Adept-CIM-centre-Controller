...
void resetEncoder(encoderReset_t encoder) { readRaw(encoder); }
...
void dac_set(enum dac_t dac_addr, uint16_t dac_voltage) {
  writeRaw(dac_addr, spindle_voltage);
  writeRaw(dac_addr + 4, spindle_voltage >> 8);

  readRaw(LOAD_DAC_ADDRESS);
}
void set_dacs(uint16_t lr_voltage, uint16_t fb_voltage, uint16_t ud_voltage,
              uint16_t spindle_voltage) {
  writeRaw(SPINDLE_DAC_LSB_ADDRESS, spindle_voltage);
  writeRaw(SPINDLE_DAC_MSB_ADDRESS, spindle_voltage >> 8);
  writeRaw(FB_DAC_LSB_ADDRESS, lr_voltage);
  writeRaw(FB_DAC_MSB_ADDRESS, lr_voltage >> 8);

  writeRaw(Y_DAC_LSB_ADDRESS, fb_voltage);
  writeRaw(Y_DAC_MSB_ADDRESS, fb_voltage >> 8);

  writeRaw(LR_DAC_LSB_ADDRESS, ud_voltage);
  writeRaw(LR_DAC_MSB_ADDRESS, ud_voltage >> 8);
...
uint16_t readFrontPanel(void) {
  ...
  ioport_set_port_level(IOPORT_PIOC, ADDRESS_BUS_MASK, 0);
  // Set the address
  ioport_set_port_level(IOPORT_PIOC,
                        ADDRESS_BUS_MASK & (FRONT_PANEL_ADDRESS << 13), 1);
  // Set MSEL low
  ioport_set_pin_level(PMSEL, 0);
  // Put read low to initiate a read
  ioport_set_pin_level(PBNIOR, 0);
  ...
  uint16_t data = ioport_get_port_level(IOPORT_PIOC, DATA_BUS_MASK) >> 1;
  ioport_set_pin_level(PBA0, 1);
  for (int i = 0; i <= 14; i++) {
    __asm__ __volatile__("nop");
  }
  // Read LSB
  data |= ioport_get_port_level(IOPORT_PIOC, DATA_BUS_MASK) << 7;
  // Put the read pin high
  ioport_set_pin_level(PBNIOR, 1);
  ioport_set_pin_level(PMSEL, 1);
  return ~data;
...
uint16_t readIO(void) {
  ...
  ioport_set_port_level(IOPORT_PIOC, ADDRESS_BUS_MASK, 0);
  // Set the address
  ioport_set_port_level(IOPORT_PIOC, ADDRESS_BUS_MASK & (IO_ADDRESS << 13), 1);
  // Set MSEL low
  ioport_set_pin_level(PMSEL, 0);
  // Put read low to initiate a read
  ioport_set_pin_level(PBNIOR, 0);
